...
bool usedInBox2(vector<vector<char>> grid,int n,int r,int c,char num){
    ...
    for(int row=0;row<3;row++){
        for(int col=0;col<3;col++){
            if((row+boxStartRow) != r && (col+boxStartCol) != c && grid[row+boxStartRow][col+boxStartCol] == num)
                return true;
        }
    }
    return false;
...
bool isSafe2(vector<vector<char>> grid,int n,int row,int col,char num){
    ...
    return !usedInRow2(grid,n,row,col,num) && !usedInCol2(grid,n,row,col,num) && !usedInBox2(grid,n,row,col,num);
}
